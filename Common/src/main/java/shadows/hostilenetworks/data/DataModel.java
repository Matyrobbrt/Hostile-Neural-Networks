...
public class DataModel extends TypeKeyedBase<DataModel> {
	...
	private static EntityType<? extends LivingEntity> byId(int id) {
		return (EntityType) BuiltInRegistries.ENTITY_TYPE.byId(id);
	}
	...
	public static DataModel read(JsonObject obj) {
		EntityType<? extends LivingEntity> t = (EntityType) BuiltInRegistries.ENTITY_TYPE.get(new ResourceLocation(obj.get("type").getAsString()));
		...
		if (obj.has("subtypes")) {
			for (JsonElement json : obj.get("subtypes").getAsJsonArray()) {
				EntityType<? extends LivingEntity> st = (EntityType) BuiltInRegistries.ENTITY_TYPE.get(new ResourceLocation(json.getAsString()));
