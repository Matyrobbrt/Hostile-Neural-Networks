...
public class DeepLearnerContainer extends PlaceboContainerMenu {
	...
	protected final Container learnerInv;
	...
	public void removed(Player pPlayer) {
		...
		HNNContainerUtil.saveItems(this.deepLearner, this.learnerInv);
	...
	public boolean hasModels() {
		...
		for (int i = 0; i < 4; i++) {
			if (!this.learnerInv.getItem(i).isEmpty()) hasModels = true;
		}
		return hasModels;
	}
	...
	public void fillWithModels(CachedModel[] models) {
		for (int i = 0; i < 4; i++) {
			ItemStack stack = this.learnerInv.getItem(i);
			models[i] = stack.isEmpty() ? null : new CachedModel(stack, i);
		}
	}

	public class DataModelSlot extends Slot {

		public DataModelSlot(Container handler, int index, int x, int y) {
