...
public class LootFabCategory implements IRecipeCategory<LootFabRecipe> {
	...
	@Override
	public void draw(LootFabRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
		Minecraft mc = Minecraft.getInstance();
		long time = mc.level.getGameTime();
		int width = Mth.ceil(36F * (this.ticks % 40 + mc.getDeltaFrameTime()) / 40);
		// TODO fix GuiComponent.blit(stack, 34, 12, 0, 30, width, 6, 256, 256);
